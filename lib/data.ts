...
// One-time self-heal: "Riley" is no longer a team member — renamed to
// "Jordan" in the seed script and default client owner, but that doesn't
// retroactively fix clients already created with the old name.
async function repairRileyOwner() {
  await prisma.client.updateMany({ where: { owner: "Riley" }, data: { owner: "Jordan" } });
}

// One-time self-heal: "Morgan" is no longer a team member — every client
// still owned or edited by Morgan transfers to Sam.
async function repairMorganAssignments() {
  await prisma.client.updateMany({ where: { owner: "Morgan" }, data: { owner: "Sam" } });
  await prisma.client.updateMany({ where: { editor: "Morgan" }, data: { editor: "Sam" } });
}
...
export async function listDashboardClients() {
  ...
  await repairRileyOwner();
  await repairMorganAssignments();
...
export async function listSchedule() {
  ...
  const tallyCrew = (clientCode: string, scopeKey: string) => {
    const owner = shootOwnerMap.get(`${clientCode}:${scopeKey}`) || "Alex";
